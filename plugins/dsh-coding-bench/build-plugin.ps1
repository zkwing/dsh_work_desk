<#
.SYNOPSIS
  Builds the standalone `dsh-coding-bench` plugin inside a DSH checkout.
...
.DESCRIPTION
  ...
  copied into <checkout>/packages/plugins/dsh-coding-bench (never into an official
  ...
  This script is a derivative of `plugins/dsh-workbench/build-plugin.ps1`. The
  only changes are: (1) every `dsh-workbench` → `dsh-coding-bench`, (2) the
  `workbench` ↔ `coding-bench` cordis id swap, and (3) the marker grep is
  relaxed to a single string — the plugin id `coding-bench` — because this
  skeleton registers no slots. The moment a real `apply` calls
  `ctx.slots.register(...)`, replace the marker list with the names the
...
$here = $PSScriptRoot
$stage = Join-Path $Checkout 'packages\plugins\dsh-coding-bench'
...
function Assert-Path([string]$Path, [string]$What) {
  if (-not (Test-Path -LiteralPath $Path)) { throw "dsh-coding-bench build: $What not found at $Path" }
}
...
if ($SkipBuild) {
  ...
  Write-Host "  & '$tsc' -p 'packages\plugins\dsh-coding-bench\tsconfig.json'   (from $Checkout)"
  Write-Host "  & '$tsdown' --config 'packages\plugins\tsdown.coding-bench.config.ts'   (from $Checkout)"
  return
}
...
try {
  & $tsc -p 'packages\plugins\dsh-coding-bench\tsconfig.json'
  if ($LASTEXITCODE -ne 0) { throw "dsh-coding-bench build: tsc exited with $LASTEXITCODE" }
...
$stagedConfig = Join-Path $Checkout 'packages\plugins\tsdown.coding-bench.config.ts'
$configSource = @"
// Generated by plugins/dsh-coding-bench/build-plugin.ps1 - do not edit in place.
//
...
const packageDir = '$CheckoutAt/packages/plugins/dsh-coding-bench'

export default {
  ...clientConfig('dsh-coding-bench', packageDir + '/lib/types/client/index.js'),
...
try {
  & $tsdown --config 'packages\plugins\tsdown.coding-bench.config.ts'
  $bundleExit = $LASTEXITCODE
} finally {
  $ErrorActionPreference = $previousPreference
  Pop-Location
}
if ($bundleExit -ne 0) { throw "dsh-coding-bench build: the browser bundle step exited with $bundleExit" }

# ── 4. verify and collect ────────────────────────────────────────────────────

$bundle = Join-Path $stage 'lib\client.js'
Assert-Path $bundle 'built client bundle'
# The plugin id is the only string the skeleton guarantees in the bundle: it
# appears in the cordis patch id, the `name` export, and the build config.
# When the real `apply` calls `ctx.slots.register(...)`, add the slot names
# the plugin claims to this list — the grep is the only way the build knows
# the closure factory actually fired.
foreach ($marker in @('coding-bench')) {
  if (-not (Select-String -LiteralPath $bundle -Pattern $marker -SimpleMatch -Quiet)) {
    throw "dsh-coding-bench build: $bundle carries no $marker registration; the client bundle did not build"
  }
}
...
if ($Clean) {
  ...
  Remove-Item -LiteralPath (Join-Path $Checkout 'packages\plugins\tsdown.coding-bench.config.ts') -Force
} else {
